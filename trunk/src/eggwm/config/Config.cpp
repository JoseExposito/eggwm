...
const char* Config::THEME_INFO           = "/theme.info";
...
void Config::reloadConfig() {
    ...
    QSettings eggwmSettings(homePath+CONFIG_FILE_PATH, QSettings::NativeFormat);

    //--------------------------------------------------------------------------

    // Abrimos el archivo de configuración del tema
    QString folderThemeName = eggwmSettings.value(USED_THEME, "").toString();
    QString themeConfigPath = homePath+THEMES_PATH+folderThemeName+THEME_INFO;
    ...
    // Guardamos el estilo
    QString themeStylePath = homePath + THEMES_PATH + folderThemeName
            + STYLE_QSS;
    QFile qss(themeStylePath);
    qss.open(QFile::ReadOnly);
    style = qss.readAll();
    style.replace("%theme_path%", homePath + THEMES_PATH + folderThemeName);
    qss.close();
}
